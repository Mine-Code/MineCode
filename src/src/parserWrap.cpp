...
parserWrap::parserWrap() : ctx(*(new parserCore)) {}
...
void parserWrap::debug() {
  ...
  for (auto [key, value] : this->ctx.variables) {
    std::wcout << "| | " << std::setw(6) << value.offset << std::setw(0)
               << " | " << std::setw(2) << value.type << std::setw(0) << " | "
               << util::str2wstr(key) << std::endl;
  }
  std::wcout << "| " << std::endl;
...
void parserWrap::set(std::wstring src) { string = src; }
void parserWrap::error_program(iterator<wchar_t> chiter) {
  std::wcerr << "Invalid Program!!!" << std::endl
